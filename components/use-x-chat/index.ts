...
>(config: XChatConfig<AgentMessage, ParsedMessage, RequestParams<Input>, Output>) {
  ...
  const getTransformMessage: TransformMessageFn<AgentMessage, Output> = (params) => {
    ...
    if (chunks?.length > 0) {
      const mergeChunk = chunk ? chunk : chunks?.[chunks?.length - 1];
      return originMessage ? originMessage : (mergeChunk as unknown as AgentMessage);
    }
    return params.chunk as unknown as AgentMessage;
  };

  const onRequest = useEvent((requestParams: AgentMessage | RequestParams<Input>) => {
    if (!agent)
      throw new Error(
