...
type ComponentToken<C extends OverrideComponent> = Exclude<
  OverrideToken[C],
  undefined
>;
    ...
    const wrapSSR = useStyleRegister(
      ...
      () => {
        ...
        const styleInterpolation = styleFn(
          mergedToken as unknown as FullToken<C>,
          { hashId, prefixCls, rootPrefixCls, iconPrefixCls },
        );
...
export const genSubStyleComponent: <C extends OverrideComponent>(
  ...
  const useStyle = genComponentStyleHook(
    ...
    styleFn,
    getDefaultToken,
    { resetStyle: false, order: -998, ...options },
  );
...
const genCSSVarRegister = <C extends OverrideComponent>(
  ...
  Object.keys(originUnitless).forEach((key) => {
    compUnitless[prefixToken(key)] =
      originUnitless[key as keyof ComponentTokenKey<C>];
  });
