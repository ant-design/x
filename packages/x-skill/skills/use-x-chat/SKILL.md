...
```ts
const errorHandler = {
  network: () => ({ content: '网络连接失败', role: 'assistant', status: 'error' }),
  abort: () => ({ content: '已取消生成', role: 'assistant', status: 'error' }),
  server: () => ({ content: '服务繁忙，请稍后重试', role: 'assistant', status: 'error' }),
};

useXChat({
  provider,
  requestFallback: (_, { error }) => {
    if (!navigator.onLine) return errorHandler.network();
    if (error.name === 'AbortError') return errorHandler.abort();
...
- **[x-chat-provider](./x-chat-provider)** - 创建自定义 Chat Provider
- **[x-request-config](./x-request-config)** - 配置 XRequest 请求参数
- **[UI集成指南](./ui-integration)** - 与 Ant Design X 组件深度集成
...
```mermaid
graph TD
    A[x-chat-provider] -->|提供 Provider 实例| B[use-x-chat]
    C[x-request] -->|配置请求| B
...
| 依赖类型 | 技能 | 说明 | 是否必须 |
| --- | --- | --- | --- |
| **核心依赖** | **x-chat-provider** | 提供自定义 Provider 实例，默认使用 XRequest | **必须** |
...
| 技能                | 作用           | 前置要求      | 后续使用                |
| ------------------- | -------------- | ------------- | ----------------------- |
| **x-chat-provider** | 创建数据适配器 | 无            | **必须**配合 use-x-chat |
| **use-x-chat**      | 管理对话状态   | 需要 Provider | 可配合 UI 组件          |
| **x-request**       | 配置网络请求   | 无            | 可配合任意技能          |
