      ...
      components: [
        {
          ...
          component: 'Stack',
          eyebrow: 'A2UI / EDGE-7',
          title: 'Autonomous Operations Surface',
          children: ['throughput', 'latency', 'confidence', 'signal', 'incident'],
        },
        {
          id: 'throughput',
          component: 'Metric',
...
const RuntimeProductionDemo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void runtime.dispatchBatch(bootstrapCommands).then((result) => {
      ...
      appendTimeline(
        'SURFACE COMMITTED',
        `${bootstrapCommands.length} A2UI commands applied atomically`,
  ...
  const runStream = async () => {
    ...
    setRunning(true);
    appendTimeline('STREAM CONNECTED', 'Receiving live dataModel patches', 'system');
    ...
    for (const [index, frame] of streamFrames.entries()) {
      ...
      const result = await runtime.dispatch(
        command({
          version: 'v0.9',
          updateDataModel: { surfaceId: SURFACE_ID, path: '/live', value: frame },
        }),
      );
      ...
      command({
        ...
        updateComponents: {
          ...
          components: [
            {
              id: 'remote-script',
              component: 'UnsafeScript',
              source: 'https://untrusted.example/payload.js',
            },
          ],
        },
      }),
            ...
            <div>
              ...
              <span>
                {latestRejection
                  ...
                  : 'Catalog + schema validation before commit'}
              </span>
            </div>
