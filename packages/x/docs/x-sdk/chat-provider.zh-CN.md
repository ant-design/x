...
如果接口需要保留推理、工具、审批、任务或 Artifact 等结构化生命周期，请实现独立的 [Agent Provider](/x-sdks/agent-provider-cn)。AgentProvider 不继承 `AbstractChatProvider`，但前端仍然通过同一个 `useXChat({ provider })` 使用。
